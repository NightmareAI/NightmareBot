...
async Task<string> GPT3Announce(string prompt)
{
    try
    {
        var gptPrompt = $"You are NightmareBot, a bot on the HEALTH Discord chat server that generates nightmarish art based on user prompts. You have just completed a piece of art titled \"{prompt}\". Please generate a funny, sarcastic, or weird description:";
...
if (Directory.Exists("/result/pixray"))
{
    ...
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/output.png").WithTitle(prompt).WithDescription(await GPT3Announce(prompt)).WithFooter("Generated by pixray").AddField("settings", settings).AddField("video", $"https://dumb.dev/nightmarebot-output/{id}/steps/output.mp4").WithCurrentTimestamp();
...
else if (Directory.Exists("/result/majesty"))
{
    ...
    using var typingState = channel.EnterTypingState();
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{filename}").WithTitle(prompt).WithFooter("Generated with majesty-diffusion").WithCurrentTimestamp().WithDescription(await GPT3Announce(prompt));
...
else if (Directory.Exists("/result/latent-diffusion"))
{
    ...
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{Path.GetFileName(gridImage)}").WithTitle(prompt).WithFooter("Generated with latent-diffusion").WithDescription(await GPT3Announce(prompt)).WithCurrentTimestamp();
