...
async Task<string> GPT3Announce(string prompt, string server, string channel, string username)
{
    try
    {
        ...
        var generated = await openAI.CompletionEndpoint.CreateCompletionAsync(gptPrompt, max_tokens: 150, temperature: 0.90, presencePenalty: 0.7, frequencyPenalty: 0.8, engine: new Engine("text-curie-001"), stopSequences: new[] { "\n\n", "\r\n\r\n"} );
...
if (Directory.Exists("/result/pixray"))
{
    ...
    var author = new EmbedAuthorBuilder().WithName(user.Username).WithIconUrl(user.GetDisplayAvatarUrl());
