...
async Task<string> GPT3Announce(string prompt, string server, string channel, string username)
{
    try
    {
        ...
        var generated = await openAI.CompletionEndpoint.CreateCompletionAsync(gptPrompt, max_tokens: 75, temperature: 0.90, presencePenalty: 0, frequencyPenalty: 0, engine: new Engine("text-curie-001"));
...
if (Directory.Exists("/result/pixray"))
{
    ...
    var author = new EmbedAuthorBuilder().WithName(user.Username).WithIconUrl(user.GetGuildAvatarUrl());
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/output.png").WithTitle(prompt).WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithFooter("Generated by pixray").AddField("settings", settings).AddField("video", $"https://dumb.dev/nightmarebot-output/{id}/steps/output.mp4").WithCurrentTimestamp().WithAuthor(author); ;
...
else if (Directory.Exists("/result/swinir"))
{
    ...
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/output.png").WithTitle(prompt).WithFooter("Enhanced with SwinIR").WithCurrentTimestamp();
...
else if (Directory.Exists("/result/majesty"))
{
    ...
    var channel = await guild.GetTextChannelAsync(channel_id);
    var user = await channel.GetUserAsync(user_id);
    var author = new EmbedAuthorBuilder().WithName(user.Username).WithIconUrl(user.GetDisplayAvatarUrl()).WithUrl(MentionUtils.MentionUser(user_id));
    using var typingState = channel.EnterTypingState();
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{filename}").WithTitle(prompt).WithFooter("Generated with majesty-diffusion").WithCurrentTimestamp().WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithAuthor(author);
...
else if (Directory.Exists("/result/latent-diffusion"))
{
    ...
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{Path.GetFileName(gridImage)}").WithTitle(prompt).WithFooter("Generated with latent-diffusion").WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithCurrentTimestamp().WithAuthor(author);
