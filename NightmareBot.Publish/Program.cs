...
if (Directory.Exists("/result/pixray"))
{
    ...
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/output.png").WithTitle(prompt.Length > 256 ? prompt.Substring(0, 256) : prompt).WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithFooter("Generated by pixray").AddField("settings", settings).AddField("video", $"https://dumb.dev/nightmarebot-output/{id}/steps/output.mp4").WithCurrentTimestamp();
