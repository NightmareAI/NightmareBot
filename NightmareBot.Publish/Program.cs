...
if (Directory.Exists("/result/pixray"))
{
    ...
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/output.png").WithTitle(prompt.Length > 256 ? prompt.Substring(0, 256) : prompt).WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithFooter("Generated by pixray").AddField("settings", settings).AddField("video", $"https://dumb.dev/nightmarebot-output/{id}/steps/output.mp4").WithCurrentTimestamp().WithAuthor(author); ;
...
else if (Directory.Exists("/result/majesty"))
{
    ...
    using var typingState = channel.EnterTypingState();
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{filename}").WithTitle(prompt.Length > 256 ? prompt.Substring(0, 256) : prompt).WithFooter("Generated with majesty-diffusion").WithCurrentTimestamp().WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithAuthor(author);
...
else if (Directory.Exists("/result/latent-diffusion"))
{
    ...
    var embed = new EmbedBuilder();
    embed.WithImageUrl($"https://dumb.dev/nightmarebot-output/{id}/{Path.GetFileName(gridImage)}").WithTitle(prompt.Length > 256 ? prompt.Substring(0, 256) : prompt).WithFooter("Generated with latent-diffusion").WithDescription(await GPT3Announce(prompt, guild.Name, channel.Name, user.Username)).WithCurrentTimestamp().WithAuthor(author);
